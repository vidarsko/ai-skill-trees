...
- **The authoring prompt asks two things before the first draft instead of guessing them:
  which language the tree should be in, and what the students get sloppy about when they
...
- **The decomposition model now says outright that a cell containing a comma goes in double
  quotes, config values included**, and its example rows show it with a `description`
  setting. The sentence it replaces ("Several fields can contain commas, so quote them
  properly") was easy to read past. Every file in the benchmark that the builder could not
  open had an unquoted comma in `,config,,description,...`. The checklist says it again.

Module versions: `authoring.json` 1.5.0 → 1.6.0, `decomposition.json` 2.7.0 → 2.8.0. Nothing
else changes.
